...
// import bukola_olusola from "../../../assets/website/images/books/WMEDSFL_contributors/bukola_olusola.jpeg";
...
const AnthologyPage = () => {
  const preorderLink = "/pre-order"; // Update with your actual preorder route
  const [selectedContributor, setSelectedContributor] = React.useState(null);
  
  // Function to format bio with paragraph breaks
  const formatBio = (bio) => {
    // Split long text into paragraphs every 2-3 sentences for readability
    ...
    <div className="anthology-page">
            ...
            {contributors.map((contributor) => (
              <div key={contributor.id} className="col-lg-3 col-md-4 col-sm-6">
                <div 
                  ...
                  onClick={() => contributor.bio.length > 100 && setSelectedContributor(contributor)}
                  style={{ cursor: contributor.bio.length > 100 ? 'pointer' : 'default' }}
                >
                  ...
                  <div className="contributor-info">
                    ...
                    <p className="contributor-bio">
                      {contributor.bio.length > 100 
                        ? `${contributor.bio.substring(0, 100)}...` 
                        : contributor.bio
                      }
                    </p>
                    {contributor.bio.length > 100 && (
                      <span className="read-more-hint">Read full bio</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <section className="anthology-final-cta">
        <div className="container text-center">
           <h3 className="contributors-title">Experience Lagos Like Never Before</h3>
